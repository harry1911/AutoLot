...
namespace AutoLot.Dal.Tests.ContextTests
{
    ...
    {
        [Fact]
        public void ShouldReturnNoCarsWithFilterNotSet()
        {
        ...
        {
            var car = Context.Cars.IgnoreQueryFilters()
                .First(x => x.Id == 1);
        ...
        public void ShouldUpdateACar()
        {
            ...
            {
                var car =
                    Context.Cars.IgnoreQueryFilters().First(c => c.Id == 1);
                Assert.Equal("Black", car.Color);
                car.Color = "White";
                Context.SaveChanges();
                ...
                var car2 = context2.Cars.IgnoreQueryFilters()
                    .First(c => c.Id == 1);
        ...
        public void ShouldUpdateACarUsingState()
        {
            ...
            void RunTheTest(IDbContextTransaction trans)
            {
                ...
                var context3 =
                    TestHelpers.GetSecondContext(Context, trans);
                var car2 = context3.Cars
                    .IgnoreQueryFilters().First(c => c.Id == 1);
                Assert.Equal("White", car2.Color);
            }
        }
        ...
        public void ShouldRemoveACar()
        {
            ...
            void RunTheTest()
            {
                var carCount = Context.Cars.IgnoreQueryFilters().Count();
                var car = Context.Cars
                    .IgnoreQueryFilters().First(c => c.Id == 2);
                Context.Cars.Remove(car);
                Context.SaveChanges();
                var newCarCount = Context.Cars
                    .IgnoreQueryFilters().Count();
                Assert.Equal(carCount - 1, newCarCount);
                Assert.Equal(
